...
main(void)
{
	...
	/* xetra example 8 */
	clob_add(c, (clob_ord_t){CLOB_TYPE_LMT, CLOB_SIDE_ASK, 0.dd, 0.0dd, .lmt = 202.dd});
	clob_add(c, (clob_ord_t){CLOB_TYPE_MKT, CLOB_SIDE_SHORT, 100.dd, 0.0dd});
	clob_add(c, (clob_ord_t){CLOB_TYPE_LMT, CLOB_SIDE_BID, 0.dd, 0.0dd, .lmt = 199.dd});
	...
	free_clob(c);
	return auc.prc != 201.dd || auc.qty != 100.dd || auc.imb != 0.dd;
}
