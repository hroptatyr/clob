...
main(void)
{
	...
	puts("LMT+BID");
	i = clob_disiter(c, CLOB_TYPE_LMT, CLOB_SIDE_BID);
	for (; clob_disiter_next(&i); j++) {
		printf("%f %f\n", (double)i.p, (double)(i.q.dis + i.q.hid));
	}
	puts("LMT+ASK");
	i = clob_disiter(c, CLOB_TYPE_LMT, CLOB_SIDE_ASK);
	for (; clob_disiter_next(&i); j++) {
		printf("%f %f\n", (double)i.p, (double)(i.q.dis + i.q.hid));
	}
	puts("MKT+BID");
	i = clob_disiter(c, CLOB_TYPE_MKT, CLOB_SIDE_BID);
	for (; clob_disiter_next(&i); j++) {
		printf("%f %f\n", (double)i.p, (double)(i.q.dis + i.q.hid));
	}
	puts("MKT+ASK");
	i = clob_disiter(c, CLOB_TYPE_MKT, CLOB_SIDE_ASK);
	for (; clob_disiter_next(&i); j++) {
		printf("%f %f\n", (double)i.p, (double)(i.q.dis + i.q.hid));
	}
